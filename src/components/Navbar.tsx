...
export default function Navbar({ menuOpen, setMenuOpen }: NavbarProps) {
  ...
  const [isClient, setIsClient] = useState(false); // État pour savoir si on est côté client
  const [scrollTop, setScrollTop] = useState(0); // Gérer la position de scroll

  // Fonction pour vérifier la position du scroll
  const checkScroll = () => {
    if (typeof window !== "undefined") {
      setScrollTop(window.scrollY); // Mettre à jour la position du scroll
      setIsScrolled(window.scrollY > 50); // Met à jour isScrolled quand on scroll plus bas que 50px
    }
  };

  useEffect(() => {
    setIsClient(true); // Mettre à jour quand on est côté client
  ...
  return (
    <>
      {isClient && ( // Vérifier qu'on est côté client avant de rendre le composant
        <nav
          className={`fixed top-0 left-0 w-full flex justify-center z-40 ${isScrolled ? 'bg-opacity-90' : 'bg-opacity-60'}`} // z-40 pour laisser le menu mobile au-dessus
          style={{ position: 'fixed', top: '0', left: '0', width: '100%', zIndex: '40' }} // z-40 ici
        >
